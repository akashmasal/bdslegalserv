...
//$origial_array['vpc_Currency'] = intval($origial_array['vpc_Currency']) * 100;
...
$sql = "UPDATE payments SET vpc_Amount=?,vpc_Locale=?,vpc_BatchNo=?,vpc_Command=?,vpc_Message=?,vpc_Version=?,vpc_Card=?,"
        . "vpc_SecureHash=?,vpc_OrderInfo=?,vpc_ReceiptNo=?,vpc_Merchant=?,vpc_AuthorizeId=?,vpc_MerchTxnRef=?,vpc_TransactionNo=?,"
        . "vpc_AcqResponseCode=?,vpc_TxnResponseCode=?,vpc_VerType=?,vpc_VerStatus=?,vpc_VerToken=?,vpc_VerSecurityLevel=?,"
        . "vpc_3DSenrolled=?,vpc_3DSXID=?,vpc_3DSECI=?,vpc_3DSstatus=? WHERE taxref=?";

$stmt = $con->prepare($sql);
$stmt->execute(array($vpc_Amount, $vpc_Locale, $vpc_BatchNo, $vpc_Command, $vpc_Message, $vpc_Version, $vpc_Card, 
    '', $vpc_OrderInfo, $vpc_ReceiptNo, $vpc_Merchant, $vpc_AuthorizeId, $vpc_MerchTxnRef, $vpc_TransactionNo,
    $vpc_AcqResponseCode, $vpc_TxnResponseCode, $vpc_VerType, $vpc_VerStatus, $vpc_VerToken, $vpc_VerSecurityLevel, $vpc_3DSenrolled, 
    $vpc_3DSXID, $vpc_3DSECI, $vpc_3DSstatus, $vpc_MerchTxnRef));
